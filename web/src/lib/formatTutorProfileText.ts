export type TutorProfileForShare = {
  id: number;
  slug: string;
  display_name: string;
  bio: string | null;
...
function priceLine(t: TutorProfileForShare): string | null {
...
export function defaultTutorProfileUrl(t: TutorProfileForShare): string {
  return `https://app.repetitor18.ru/tutors/${t.id}-${t.slug}`;
}

export function formatTutorProfileText(t: TutorProfileForShare, profileUrl?: string): string {
  const lines: string[] = [];
  ...
  lines.push(`🔗 Анкета: ${profileUrl || defaultTutorProfileUrl(t)}`);
  return lines.join("\n");
}
